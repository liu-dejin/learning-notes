...
![多学一招](E:\Learn\java-code\day03-运算符\笔记\img\多学一招.jpg)
...
表示直观、易懂，与真值转换容易。
0有两种不同的表示形式，给使用带来了不便。通常0的原码用+0表示，若在计算过程中出现了-0，则需要用硬件将-0变成+0。
原码表示加减运算复杂。
## 反码
...
用符号位表示数值的正负，形式与原码表示相同，即0为正；1为负。
数值0有两种表示方法。
反码的表示范围与原码的表示范围相同。
